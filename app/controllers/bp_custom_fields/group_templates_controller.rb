  ...
  class GroupTemplatesController < ApplicationController
    ...
    private
      ...
      def group_template_params
        params.require(:group_template).permit(:name, :visible, field_templates_attributes: [:_destroy, :field_type, :required, :min, :max, :prepend, :required, :default_value, :instructions, :label, :placeholder_text, :id, :name], appearances_attributes: [:_destroy, :id, :resource, :resource_id, :appears])
      end
  end
end
